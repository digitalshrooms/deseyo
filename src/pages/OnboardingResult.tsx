...
export const OnboardingResult = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center py-12 px-4 relative overflow-hidden" style={{ backgroundColor: 'var(--bg)' }}>
      ...
      <div className="relative z-10 max-w-2xl w-full">
        <div className="rounded-2xl p-8 md:p-12" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}>
          <div className="flex justify-center mb-6">
            <div className="w-16 h-16 rounded-full flex items-center justify-center" style={{ backgroundColor: 'var(--primary-soft)' }}>
              <CheckCircle className="w-10 h-10" style={{ color: 'var(--primary)' }} />
            </div>
          </div>

          <h1 className="font-normal text-center mb-2" style={{ fontSize: 'clamp(24px, 5vw, 34px)', color: 'var(--text)' }}>
            Pro tebe teď dává největší smysl začít tady:
          </h1>

          <div className="text-center mb-8">
            <h2 className="font-normal mb-2" style={{ fontSize: 'clamp(32px, 7vw, 44px)', color: 'var(--primary)' }}>{details.title}</h2>
            <p className="text-lg" style={{ color: 'var(--text-muted)' }}>{details.subtitle}</p>
          </div>

          <div className="rounded-xl p-6 mb-8" style={{ backgroundColor: 'var(--primary-soft)', border: '1px solid var(--primary)' }}>
            <p className="leading-relaxed" style={{ color: 'var(--text)' }}>{details.description}</p>
          </div>

          <div className="mb-8">
            <h3 className="text-lg font-normal mb-4" style={{ color: 'var(--text)' }}>Co tě čeká:</h3>
          ...
          {pricing && user?.subscription_status !== 'active' && (
            <div className="rounded-xl p-6 mb-6" style={{ backgroundColor: 'var(--bg-elevated)', border: '1px solid var(--border)' }}>
              <div className="flex items-center justify-between mb-3">
                <span style={{ color: 'var(--text-muted)' }}>Členství {details.title}</span>
                <span className="text-2xl font-normal" style={{ color: 'var(--text)' }}>{pricing.label}</span>
              </div>
          ...
          {!pricing && (
            <div className="rounded-xl p-6 mb-6" style={{ backgroundColor: 'var(--bg-elevated)' }}>
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full text-white py-4 rounded-full font-normal transition-all hover:opacity-90 active:scale-[0.98] flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
