...
export function DailyOnboardingModal({ userId, currentDay, onClose }: DailyOnboardingModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div
        className="themed-scrollbar rounded-3xl shadow-2xl w-full max-w-2xl max-h-[92vh] overflow-y-auto relative"
        style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}
        onClick={(e) => e.stopPropagation()}
      >
        {/* ── Progress Map ── */}
        <div className="px-6 pt-6 pb-4" style={{ background: `linear-gradient(135deg, ${phase.color}26, transparent 70%)` }}>
          <div className="flex items-center justify-between mb-3">
            <div>
              <p className="text-xs font-normal uppercase tracking-widest" style={{ color: phase.color }}>
                {phase.name}
              </p>
              <h2 className="font-normal mt-0.5" style={{ fontSize: 'clamp(20px, 4vw, 26px)', color: 'var(--text)' }}>
            ...
            <button
              onClick={handleSkip}
              disabled={submitting}
              className="text-xs flex items-center gap-1 px-2 py-1 rounded-lg transition-colors hover:opacity-70"
          ...
          <div className="relative mt-5 mb-2">
            <div className="flex items-center h-2 rounded-full overflow-hidden" style={{ backgroundColor: 'var(--bg-elevated)' }}>
              {PHASES.map((p, i) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      width: `${width}%`,
                      backgroundColor: isComplete || isCurrent ? p.color : 'var(--bg-elevated)',
                ...
                return (
                  ...
                  >
                    <div
                      ...
                      style={{
                        backgroundColor: isCurrent ? phase.color : isPassed ? phase.color : 'var(--border-strong)',
                    ...
                    <span
                      ...
                      style={{ color: isCurrent || isPassed ? phase.color : 'var(--text-subtle)' }}
                    >
                      D{m}
                    </span>
                  </div>
          ...
          <div className="flex items-center gap-2 mt-5">
            <span className="text-xs font-normal" style={{ color: 'var(--text-subtle)' }}>Tento týden:</span>
              ...
              {weekDays.map((d) => {
                ...
                return (
                  ...
                  >
                    <button
                      onClick={() => goToDay(d)}
                      className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-normal transition-all hover:scale-110"
                      style={{
                        backgroundColor: isViewing ? phase.color : isPast ? phase.color : isToday ? phase.color : 'transparent',
                        color: isViewing ? '#fff' : isPast ? '#fff' : isToday ? '#fff' : 'var(--text-subtle)',
                        border: isViewing ? `2px solid ${phase.color}` : isToday ? `2px solid ${phase.color}` : d > day ? '1.5px solid var(--border)' : 'none',
                      }}
                    >
        ...
        {/* ── Day navigation ── */}
        <div className="flex items-center justify-between px-6 py-2" style={{ borderBottom: '1px solid var(--border)' }}>
          <button
            onClick={() => goToDay(viewingDayClamped - 1)}
            disabled={viewingDayClamped <= 1}
            className="flex items-center gap-1 text-sm font-normal disabled:opacity-30 disabled:cursor-not-allowed transition-colors hover:opacity-70"
        ...
        <div className="px-6 pb-6">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-sm mt-3" style={{ color: 'var(--text-subtle)' }}>Načítám obsah...</p>
            </div>
          ) : error ? (
            <div className="text-center py-12">
              ...
              <button
                onClick={onClose}
                className="mt-4 px-6 py-2.5 rounded-xl text-sm font-normal text-white transition-opacity hover:opacity-90"
            ...
            <>
              ...
              {requiresVideoWatch && !videoWatched && (
                <div className="flex items-center gap-2 rounded-xl px-4 py-2.5 mb-4" style={{ backgroundColor: 'rgba(245, 158, 11, 0.1)', border: '1px solid rgba(245, 158, 11, 0.3)' }}>
                  <PlayCircle className="w-4 h-4 flex-shrink-0" style={{ color: 'var(--warning)' }} />
                  <p className="text-xs font-normal" style={{ color: 'var(--warning)' }}>
              ...
              {requiresVideoWatch && videoWatched && !content.post_video_message && (
                <div className="flex items-center gap-2 rounded-xl px-4 py-2.5 mb-4" style={{ backgroundColor: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.3)' }}>
                  <Check className="w-4 h-4 flex-shrink-0" style={{ color: 'var(--success)' }} />
                  <p className="text-xs font-normal" style={{ color: 'var(--success)' }}>Video dokončeno — můžete odpovědět.</p>
                </div>
              )}

              {/* Post-video inspirational message (e.g. D5) */}
              {content.post_video_message && videoWatched && (
                <div className="rounded-2xl px-5 py-4 mb-4" style={{ backgroundColor: 'var(--primary-soft)', border: '1px solid var(--primary)' }}>
                  <p className="text-base font-normal leading-relaxed text-center" style={{ color: 'var(--primary-accent-text)' }}>
              ...
              {viewingDayClamped === 3 && videoWatched && content.linked_course_id && (
                <div className="rounded-2xl px-4 py-4 mb-4" style={{ backgroundColor: 'rgba(244, 63, 94, 0.08)', border: '1px solid rgba(244, 63, 94, 0.25)' }}>
                  <div className="flex items-start gap-3 mb-3">
                    <Heart className="w-5 h-5 flex-shrink-0 mt-0.5" style={{ color: ROSE }} />
                    <div>
                      <p className="text-sm font-normal mb-0.5" style={{ color: ROSE }}>Sedla ti lekce?</p>
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl font-normal text-sm transition-all"
                    style={
                      isFavorite
                        ? { backgroundColor: ROSE, color: '#fff' }
                        : { backgroundColor: 'var(--bg-card)', border: `2px solid ${ROSE}`, color: ROSE }
                    }
                  >
              ...
              {content.download_url && (
                <div className="mb-5">
                  <div className="rounded-2xl p-5" style={{ backgroundColor: 'var(--primary-soft)', border: '1px solid var(--primary)' }}>
                    <div className="flex items-start gap-3 mb-4">
                      ...
                      <div className="min-0">
                        <h3 className="text-base font-normal mb-1" style={{ color: 'var(--text)' }}>{technique?.title || 'Workbook'}</h3>
                        {content.question_text && (
                          <p className="text-sm leading-relaxed" style={{ color: 'var(--text-muted)' }}>{content.question_text}</p>
                        )}
                      </div>
                    </div>
                    ...
                    {canAnswer ? (
                      <div className="flex gap-2">
                        <a
                          ...
                          className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-normal text-sm transition-colors hover:opacity-80"
                          style={{ backgroundColor: 'var(--bg-card)', border: '2px solid var(--primary)', color: 'var(--primary-accent-text)' }}
                        >
                          Náhled
                        </a>
                        <a
                          href={content.download_url}
                          download
                          className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-normal text-sm text-white transition-opacity hover:opacity-90"
                          style={{ backgroundColor: 'var(--primary)' }}
                        >
                      ...
                      <div className="w-full flex items-center justify-center gap-2 py-3 rounded-xl font-normal text-sm cursor-not-allowed" style={{ backgroundColor: 'var(--bg-elevated)', color: 'var(--text-subtle)' }}>
                    ...
                    {technique && (
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl font-normal text-sm mt-2.5 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                        style={
                          isTechniqueSaved
                            ? { backgroundColor: 'var(--primary-soft)', border: '2px solid var(--primary-light)', color: 'var(--primary-accent-text)' }
                            : { backgroundColor: 'var(--bg-card)', border: '2px solid var(--primary-light)', color: 'var(--primary-accent-text)' }
                        }
                      >
                        <Heart className={`w-4 h-4 ${isTechniqueSaved ? 'fill-current' : ''}`} />
              ...
              {!isDownloadOnly && !isVideoOnly && !isVideoWithDownload && (
                <div className="mb-4">
                  <h3 className="text-lg font-normal mb-3 transition-colors" style={{ color: canAnswer ? 'var(--text)' : 'var(--text-subtle)' }}>
                    {content.question_text}
                  </h3>
                  {!isCurrentDay && (
                    <p className="text-xs mb-3 italic" style={{ color: 'var(--text-subtle)' }}>
                  ...
                  {isOpenEnded ? (
                    <textarea
                      ...
                      rows={3}
                      className={`w-full px-4 py-3 rounded-2xl border-2 transition-all outline-none text-sm font-normal resize-none ${submitting ? 'opacity-50' : ''}`}
                      style={
                        canAnswer
                          ? { borderColor: 'var(--border)', backgroundColor: 'var(--bg-card)', color: 'var(--text)' }
                          : { borderColor: 'var(--border)', backgroundColor: 'var(--bg-elevated)', color: 'var(--text-subtle)', cursor: 'not-allowed' }
                      }
                    />
                  ) : (
                      ...
                      {content.options.map((option, index) => {
                        ...
                        return (
                          <button
                            ...
                            disabled={submitting || !canAnswer}
                            className={`w-full text-left px-4 py-3.5 rounded-2xl border-2 transition-all flex items-center justify-between group ${submitting ? 'opacity-50' : ''}`}
                            style={
                              isSelected
                                ? { borderColor: 'var(--primary)', backgroundColor: 'var(--primary-soft)' }
                                : canAnswer
                                  ? { borderColor: 'var(--border)', backgroundColor: 'var(--bg-card)' }
                                  : { borderColor: 'var(--border)', backgroundColor: 'var(--bg-elevated)', cursor: 'not-allowed' }
                            }
                          >
                            <span className="text-sm font-normal" style={{ color: isSelected ? 'var(--primary-accent-text)' : canAnswer ? 'var(--text)' : 'var(--text-subtle)' }}>
                              {option}
                            </span>
                            <div
              ...
              {isCurrentDay ? (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl text-white font-normal text-sm transition-all disabled:opacity-40 disabled:cursor-not-allowed hover:opacity-90"
                  style={{ backgroundColor: 'var(--primary)' }}
                >
                  {submitting ? (
                    <>
                ...
                <button
                  onClick={() => goToDay(day)}
                  className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl text-white font-normal text-sm transition-all hover:opacity-90"
                  style={{ backgroundColor: 'var(--primary)' }}
                >
                  {buttonLabel}
                  <ChevronRight className="w-4 h-4" />
                ...
                <div className="space-y-2">
                  ...
                  <button
                    onClick={() => goToDay(day)}
                    className="w-full flex items-center justify-center gap-2 py-2.5 rounded-2xl font-normal text-sm transition-all hover:opacity-70"
                    style={{ color: 'var(--text-muted)' }}
                  >
                    Zpět na dnešní den
                  </button>
